...
exports.create = async (payload) => {
    ...
    return repo.create({...value,locations: payload.locations.join(','),ethiopia_locations: payload.ethiopia_locations.join(','),platforms: payload.platforms.join(',')});
}
...
exports.update = async (id, payload) => {
    ...
    await repo.update(id, {...value,locations: payload.locations.join(','),ethiopia_locations: payload.ethiopia_locations.join(','),platforms: payload.platforms.join(',')});
...
exports.updateStatus = async (id, payload, currentUser) => {
    ...
    if (campaign.target_type == "community" && value.status == "accepted") {
        const community = repo.findCommunity(campaign.target_id)
        if (!community) throw new Error("Community not found");
        value.commission_rule_type = community.commission_type
        value.commission_rule_value = community.commission_type == "Rate" ? community?.commission_rate : community.commission_amount
    }
