...
export default function CreateCampaign() {
  ...
  const populateFromCampaign = (campaign) => {
    ...
    setForm({
      ...
      EthiopiaLc: campaign.ethiopia_locations,
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      console.log("Campaign data:", res?.data?.data || res?.data);
      if (res?.success) {
  ...
  const toggleLocation = (location) => {
    setForm((prev) => {
      const isIncluded = prev.locations?.split(",")?.includes(location);
      return {
        ...prev,
        locations: isIncluded
          ? prev.locations?.split(",").filter((item) => item !== location).join(",")
          : prev.locations ? [...(prev.locations?.split(",") || []), location].join(",") : location,
      };
    });
  ...
  const ethiopianLocation = (location) => {
    setForm((prev) => {
      const isIncluded = prev.EthiopiaLc?.split(",")?.includes(location);
      return {
        ...prev,
        EthiopiaLc: isIncluded
          ? prev.EthiopiaLc?.split(",")?.filter((item) => item !== location).join(",")
          : prev.EthiopiaLc ? [...(prev.EthiopiaLc.split(",")), location].join(",") : location,
      };
    });
  };
  ...
  const buildPayload = (isPublish = false) => {
    ...
    payload.append(
      ...
      isPublish
        ? (
          isEditMode
            ? "Campaign updated and submitted for approval successfully."
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                      {EthiopiaLc?.map((location) => (
                        <Checkbox
                          key={location}
                          label={location}
                          checked={form.EthiopiaLc?.split(",").includes(location)}
                ...
                <ReviewSection title="Audience & Platforms" icon={MapPin}>
                  <div className="py-2">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {form.locations.split(",")?.length ? (
                        form.locations?.trim("")?.split(",").map((l) => l && <Chip key={l}>{l}</Chip>)
                      ) : (
                  ...
                  {form.EthiopiaLc?.length > 0 && (
                    <div className="py-2 border-t border-gray-100">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {form.EthiopiaLc?.trim(" ")?.split(",").map((l) => (
                          l && <Chip key={l}>{l}</Chip>
                        ))}
                      </div>
                    </div>
