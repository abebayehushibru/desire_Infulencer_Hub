...
const SAMPLE_COMMUNITY = {
  ...
  location: "Ethiopia",
  status: "Active",
  existingCoverImage: "sara-beauty-cover.jpg",
  existingAvatarImage: "sara-beauty-avatar.jpg",

  about: "Sara Beauty Community brings together creators, MUAs, and skincare enthusiasts across Ethiopia to collaborate on brand campaigns and grow together.",
  goals: "Grow to 20K members by year end. Run 50 brand campaigns in 2026. Keep engagement above 8%.",
  rules: "Be respectful, no spam or self-promo. Disclose sponsored content. Keep posts on-topic.",
  categories: ["Beauty", "Lifestyle", "Fashion"],
  platforms: ["Instagram", "TikTok", "Telegram", "YouTube"],

  visibility: "Public",
  memberApproval: "Manual",
  postApproval: true,
  notifications: true,
  manager: "Sara Beauty (Owner)",
};
...
export default function EditCommunity({ community = SAMPLE_COMMUNITY }) {
  const [step, setStep] = useState(0);
  const [form, setForm] = useState({ ...EMPTY_FORM, ...community });
  ...
  const validateStep = (s) => {
    ...
    if (s === 0) {
      ...
      if (!form.avatarImage && !form.existingAvatarImage) e.avatarImage = "Community avatar is required.";
    }
    if (s === 1) {
  ...
  const submit = () => {
    const payload = {
      ...
      profile: {
        ...
        avatar_image: form.avatarImage || undefined,
        cover_image: form.coverImage || undefined,
      },
      settings: {
        visibility: form.visibility,
        member_approval: form.memberApproval,
        post_approval: form.postApproval,
        notifications: form.notifications,
      },
    };
    console.log("Update Community payload:", payload);
    setSubmitted(true);
  };
    ...
    return (
      <div style={BRAND} className="mx-auto flex min-h-[480px] w-full max-w-2xl flex-col items-center justify-center gap-3 rounded-2xl border border-slate-200 bg-white p-10 text-center shadow-sm">
        ...
        <p className="max-w-sm text-sm text-slate-500">
          {form.name || "This community"}'s details have been updated and it's now {form.status.toLowerCase()}.
        </p>
        <button
      ...
      {step === 3 && (
        <div className="space-y-5">
          <ReviewSection title="Community Details">
            ...
            <ReviewRow
              label="Avatar"
              value={form.avatarImage ? `${form.avatarImage.name} (new)` : form.existingAvatarImage || "—"}
            />
            <ReviewRow
              label="Cover Image"
          ...
          <ReviewSection title="Settings">
            ...
            <ReviewRow label="Commission Value" value={form.commissionValue ? `${form.commissionType === "Rate" ? `${form.commissionValue}%` : `$${form?.commissionValue?.toFixed(2)}`}` : "—" } />
