...
function FileDrop({ label, value, onChange, optional }) {
  return (
...
export default function CreateInfluencer() {
  ...
  if (submitted) {
    return (
      <div
        style={brandVars}
        className="w-full flex min-h-[480px] w-full  flex-col items-center justify-center gap-3 rounded-2xl border border-slate-200 bg-white p-10 text-center shadow-sm"
      >
        ...
        <h2 className="text-lg font-semibold text-slate-800">Influencer created</h2>
        <p className="max-w-sm text-sm text-slate-500">
          {form.fullName || "The new influencer"} has been added and is now {form.status.toLowerCase()}.
        </p>
        <button
          onClick={() => {
            setForm(initialForm);
  ...
  return (
    ...
    >
      <div className="mb-6">
        <h1 className="text-xl font-semibold tracking-tight text-slate-800">Create Influencer</h1>
        <p className="text-sm text-slate-500">Add a new influencer profile in three quick steps.</p>
      </div>
      ...
      {step === 3 && (
        <div className="space-y-5">
          ...
          <ReviewSection title="Influencer Profile">
            ...
            <ReviewRow label="Profile Photo" value={form.profilePhoto ? form.profilePhoto.name : "—"} />
            <ReviewRow label="National ID" value={form.nationalId ? form.nationalId.name : "Not provided"} />
          </ReviewSection>
